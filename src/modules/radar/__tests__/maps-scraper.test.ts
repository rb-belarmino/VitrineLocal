...
describe('MapsScraper (Unit & Integration)', () => {
  ...
  it('deve extrair dados e respeitar o limite solicitado via página mockada', async () => {
    ...
    vi.spyOn(BrowserPool, 'getBrowser').mockResolvedValue(
      mockBrowser as unknown as import('playwright').Browser,
    );
    vi.spyOn(BrowserPool, 'createStealthContext').mockResolvedValue(
    ...
    expect(mockPage.goto).toHaveBeenCalledWith(
      expect.stringContaining(
        'google.com/maps/search/Oficina%20Mec%C3%A2nica%20em%20Moema%2C%20S%C3%A3o%20Paulo',
      ),
      expect.any(Object),
    );
